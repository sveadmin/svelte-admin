...
export function normalizeVisibleSizeAsData(
  ...
  if (direction === SIZE_DIRECTION_HORIZONTAL) {
    switch (visibleSize.unit) {
      case VISIBLE_SIZE_UNIT_SPAN:
        return {
          'grid-column-end': 'span ' + visibleSize.size
        }
        
      case VISIBLE_SIZE_UNIT_CHARACTERS:
        return {
          width: 'calc(' + visibleSize.size + ' * var(--width-factor))'
        }
      default:
        return {
          width: visibleSize.size + (visibleSize.unit ?? '')
        }
    }
  } else {
    switch (visibleSize.unit) {
      case VISIBLE_SIZE_UNIT_SPAN:
        return {
          'grid-row-end': 'span ' + visibleSize.size
        }
      case VISIBLE_SIZE_UNIT_CHARACTERS:
        return {
          height: 'calc(' + visibleSize.size + ' * var(--height-factor))'
        }
      default:
        return {
          height: visibleSize.size + (visibleSize.unit ?? '')
        }
    }
  }
