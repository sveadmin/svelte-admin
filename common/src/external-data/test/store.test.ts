...
describe('Test external data store', () => {
  it('External data behaves as singleton', async () => {
    ...
    expect(store2.getKey('test')).toBe('valueTest')

    store1.set({
      'test2': 'valueTest2'
    })
    expect(store2.getKey('test2')).toBe('valueTest2')
    expect(store2.getKey('test')).toEqual({})
  })
  it('Has function works', async () => {
    ...
    store1.getKey('test') //By default the keys are removed on first query
  ...
  it('Default works', async () => {
    ...
    const test = store1.getKey('notSet', 'defaultValue')
